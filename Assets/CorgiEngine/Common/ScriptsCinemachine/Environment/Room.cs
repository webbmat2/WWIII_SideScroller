...
namespace MoreMountains.CorgiEngine
{
	...
	public class Room : CorgiMonoBehaviour, MMEventListener<CorgiEngineEvent>
	{
		...
		protected virtual IEnumerator CameraInitialization()
		{
			yield return null;
			yield return null;
			if (!CurrentRoom)
		...
		protected virtual IEnumerator ResizeConfiner()
		{
			...
			#elif MM_CINEMACHINE3
			if ((VirtualCamera == null) || (Confiner2D == null) || !ResizeConfinerAutomatically)
			{
				yield break;
			}
			...
			(Confiner2D as BoxCollider2D).offset = _roomCollider2D.offset;
			(Confiner2D as BoxCollider2D).size = (_roomCollider2D as BoxCollider2D).size;

			_cameraSize.y = 2 * _mainCamera.orthographicSize;
			_cameraSize.x = _cameraSize.y * _mainCamera.aspect;

			Vector2 newSize = (Confiner2D as BoxCollider2D).size;

			if ((Confiner2D as BoxCollider2D).size.x < _cameraSize.x)
			{
				newSize.x = _cameraSize.x;
			}
			if ((Confiner2D as BoxCollider2D).size.y < _cameraSize.y)
			{
				newSize.y = _cameraSize.y;
			}

			(Confiner2D as BoxCollider2D).size = newSize;
